...
import { ArquivoPermissaoInterface, ArquivoInterface } from '../_models';
...
export class ArquivoService {
  ...
  // public _arquivos = new BehaviorSubject<any[]>([]); // ARQUIVOS JÁ CADASTRADOS
  private icons = {
    image: 'jpg',
    pdf: 'pdf',
    word: 'doc',
  ...
  public getPreUrlDownload(nomeS3): Observable<string>  {
    const dados = {
      'nomeS3': nomeS3
    };
  ...
  public getDonloadS3(url) {
    return this.http.get(url, { responseType: 'blob'});
  }

  public getDownload(nome, nomeS3) {
    this.ass.mostraCarregador();
  ...
  public atualisaCotaUpload(valor) {
  ...
  private deleteArquivoServer(id): Observable<boolean> {
    const url = this.url.arquivo + '/' + id;
    return this.http.delete<boolean>(url);
  }

  public deleteArquivo(arquivo: ArquivoInterface): Observable<boolean> {
    const arquivo_id: number = arquivo.arquivo_id;
    const arquivo_tamanho: number = arquivo.arquivo_tamanho;
    let vf: boolean;
  ...
  private arquivoDelete(arquivo_id) {
  ...
  public atualisaCotaDelete(valor) {
    const dados: ArquivoPermissaoInterface = JSON.parse(sessionStorage.getItem('arquivo-permissoes'));
    dados.cota_disponivel += valor;
    dados.cota_utilizada -= valor;
    sessionStorage.removeItem('arquivo-permissoes');
  ...
  getClassNameForExtension(name: string): string {
    ...
    const str = '/assets/img/icones/' + ic.toLowerCase() + '.png';
    return str;
  }
